...
export default function BenchmarkDetailPageComponent({ benchmarkId, onRunningChange, navSidebar, rightSidebar }: any) {
  ...
  const handleRun = useCallback(async () => {
    ...
    const agentTargets = agentInstances
      ...
      .map((a: any) => {
        const modelDef = allModels.find((m: any) => m.provider === a.provider && m.name === a.modelName);
        return {
          provider: a.provider,
          model: a.modelName,
  ...
  const handleStop = useCallback(async () => {
    ...
    if (streamingResults.length > 0) {
      ...
      const failed = streamingResults.filter((r: any) => !r.passed && !r.error).length;
      const errored = streamingResults.filter((r: any) => r.error).length;
      const totalCost = streamingResults.reduce((s: any, r: any) => s + (r.estimatedCost || 0), 0);
      setLatestRun({
        id: "partial-" + Date.now(),
        aborted: true,
  ...
  // -- View a past run ----------------------------------------
  const viewRun = useCallback((run: any) => {
    setLatestRun(run);
    setActiveRunId(run.id);
    setSelectedResult(null);
  ...
  const handleAddAgent = useCallback((agentDef: any) => {
    const instance = {
      instanceId: generateUUID(),
      agentId: agentDef.id,
      name: agentDef.name,
  ...
  const removeAgent = useCallback((instanceId: any) => {
    setAgentInstances((prev: any) => {
    ...
    setThinkingMap((prev: any) => { const n = { ...prev }; delete n[instanceId]; return n; });
  }, [selectedInstances, thinkingMap, toolsMap]);

  const handleChangeAgentModel = useCallback((instanceId: any, provider: any, modelName: any) => {
    setAgentInstances((prev: any) => {
  ...
  const handleModelSelect = useCallback((rawModel: any) => {
    const instance = {
      instanceId: generateUUID(),
  ...
  const removeModel = useCallback((instanceId: any) => {
    setSelectedInstances((prev: any) => {
      const next = prev.filter((i: any) => i.instanceId !== instanceId);
      StorageService.set(SK_MODEL_MEMORY_BENCHMARKS, {
        instances: next,
    ...
    setThinkingMap((prev: any) => { const n = { ...prev }; delete n[instanceId]; return n; });
    setToolsMap((prev: any) => { const n = { ...prev }; delete n[instanceId]; return n; });
  }, [agentInstances, thinkingMap, toolsMap]);

  const handleChangeModel = useCallback((instanceId: any, provider: any, modelName: any) => {
    setSelectedInstances((prev: any) => {
            ...
            <>
              ...
              <ButtonComponent
                ...
                disabled={(() => {
                  ...
                  if (m === "model") return !form.assertions?.some((a: any) => a.expectedValue);
                  if (m === "agent") return !form.agentAssertions?.length;
                  return !form.assertions?.some((a: any) => a.expectedValue) && !form.agentAssertions?.length;
                })()}
              >
                Create Clone
              </ButtonComponent>
