...
let colorIdx = 0;
function getQuantColor(q: string) {
  if ((QUANT_COLORS as Record<string, { bg: string; border: string }>)[q]) return (QUANT_COLORS as Record<string, { bg: string; border: string }>)[q];
  const fallbackColor = PALETTE[colorIdx % PALETTE.length];
  colorIdx++;
  return fallbackColor;
}

function getGPUColor(gpuName: string) {
  return (
    (GPU_COLORS as Record<string, { bg: string; border: string }>)[gpuName] || {
...
function makeDatalabelsPlugin({
  ...
  getLabel: (raw: VramBenchmarkEntry, i: number, di: number) => string | null | undefined;
...
function makeConnectorHighlightPlugin() {
  return {
    ...
    afterDraw(chart: Chart) {
      const hoveredModel = (chart as any)._hoveredConnectorModel;
      ...
      for (let di = 0; di < chart.data.datasets.length; di++) {
        ...
        for (let i = 0; i < ds.data.length; i++) {
          ...
          bubblePoints.push({
            x: element.x,
            y: element.y,
            r: element.options?.radius || (raw as any).r || 5,
...
function SettingsTooltipContent({ settingsKey }: { settingsKey: string }) {
  const info = (SETTINGS_INFO as Record<string, { flash: boolean; kv: string; batch: number; parallel: number; purpose: string }>)[settingsKey];
      ...
      >
        ...
        {rows.map(([key, info]) => (
          <Fragment key={key}>
            ...
            <span>{(info as { flash: boolean }).flash ? "✓" : "✗"}</span>
            <span>{(info as { kv: string }).kv}</span>
            <span>{(info as { batch: number }).batch}</span>
            <span>{(info as { parallel: number }).parallel}</span>
          </Fragment>
        ))}
      </span>
    </span>
...
export default function VramBenchmarkComponent() {
  ...
  const searchOrigColors = useRef<Record<string, string>>({});
  ...
  const renderScatter = useCallback(() => {
    ...
    const existing = (chartInstances.current as any).scatter;
    ...
    const toPoint = (m: VramBenchmarkEntry) => {
      const x = mode.getX(m);
      const y = mode.getY(m);
    ...
    if (machineFilter === "all") {
      ...
      const modelToPoints: Record<string, { x: number, y: number, r: number, model: VramBenchmarkEntry }[]> = {};
      for (const m of scatterModels) {
        if (bestKeys && !bestKeys.has(entryKey(m))) continue;
        const pt = toPoint(m);
        if (!pt) continue;
        const key = m.displayName || m.model;
      ...
      for (const [, points] of Object.entries(modelToPoints)) {
        ...
        datasets.push({
          type: "line",
          label: "_connector",
          data: points.map((p: any) => ({ x: p.x, y: p.y, model: p.model })),
      ...
      if (showAllCtx) {
        ...
        source = models.map((m: any) => m); // clone so filter doesn't mutate
      }
      ...
      for (const m of scatterData) {
        ...
        (quantGroups as any)[q].push(m);
      }
      datasets = Object.entries(quantGroups).map(([q, items]: any) => {
    ...
    const currentChart = (chartInstances.current as any).scatter;
    if (currentChart) {
      ...
      (chartInstances.current as any).scatter = new Chart(context, {
        ...
        plugins: [
          makeDatalabelsPlugin({
            getLabel: (raw: any) => shortModelName(raw?.model?.displayName, 16),
            anchor: "end",
            align: "top",
            offset: 4,
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: (item: { raw?: unknown }) => {
                  ...
                  if ((m.vramDuringGen as any)?.peakGiB)
                    lines.push(
                      `Peak VRAM (gen): ${((m.vramDuringGen as any).peakGiB as number).toFixed(2)} GiB`,
                    );
                  if (m.ttft?.ms) {
                  ...
                  if ((m.gpu as any)?.temp)
                    lines.push(
                      `GPU: ${(m.gpu as any).temp}°C · ${(m.gpu as any).power || "?"}W · ${(m.gpu as any).utilization || "?"}%`,
                    );
  ...
  const { vramRanges, tpsRanges, ctxRanges } = useMemo(() => {
    ...
    for (const d of source) {
      ...
      const t = (d as any).tokensPerSecond || 0;
      ...
      if (!(vram as any)[name]) {
        ...
        (vram as any)[name].count++;
        (vram as any)[name].values.push(v);
        (vram as any)[name].entries.push(d);
      }
      ...
      if (t > 0) {
        if (!(tps as any)[name]) {
          ...
          (tps as any)[name].count++;
          (tps as any)[name].values.push(t);
          (tps as any)[name].entries.push(d);
        }
      }
      ...
      if (c > 0) {
        ...
        if (!(ctxR as any)[name]) {
          ...
          (ctxR as any)[name].count++;
          (ctxR as any)[name].values.push(cK);
          (ctxR as any)[name].entries.push(d);
        }
      }
    }
  ...
  useEffect(() => {
    const chart = (chartInstances.current as any).scatter;
    if (!chart) return;
  ...
  useEffect(() => {
    const chart = (chartInstances.current as any).bar;
    if (!chart) return;
    const xScale = chart.options.scales.x;
  ...
  const renderBar = useCallback(() => {
    ...
    const allVram = models.map((m: any) => m.modelVramGiB);
    const vMin = Math.min(...allVram);
    const vMax = Math.max(...allVram);
    const vSpan = vMax - vMin || 1;

    function vramColor(gib: any, alpha = 0.55) {
      ...
      plugins: [
        makeDatalabelsPlugin({
          getLabel: (_raw: any, i: any) => {
            const m = models[i];
            if (!m) return "";
            const range = (vramRanges as any)[(m as any).displayName];
            if (range && range.count > 1) {
              return `${range.min.toFixed(1)}–${range.max.toFixed(1)}G`;
            }
            return `${(m as any).modelVramGiB.toFixed(1)}G`;
          },
          anchor: "end",
          align: "right",
          offset: 6,
          filterFn: (di: any) => di === 0,
        }),
      ],
      options: {
        indexAxis: "y",
          ...
          tooltip: {
            ...
            callbacks: {
              title: (items: any) => {
                ...
                return (models[item.dataIndex] as any)?.displayName || "";
              },
              afterTitle: (items: any) => {
                ...
                if (!m) return "";
                return `${(m as any).quantization} · ${(m as any).architecture} · ${((m as any).contextLength / 1024).toFixed(0)}K ctx · ${shortGPU((m as any).system?.gpu?.name)}`;
              },
              label: (item: any) => {
                ...
                const m = models[item.dataIndex];
                const range = (vramRanges as Record<string, any>)[m?.displayName || ""];
                if (range && range.count > 1) {
  ...
  const renderEfficiency = useCallback(() => {
    ...
    const sorted = [...models].sort(
      (a: any, b: any) => b.tokensPerSecond - a.tokensPerSecond,
    );

    const labels = sorted.map((m: any) => {
      const name = m.displayName;
      return name.length > 30 ? name.slice(0, 28) + "…" : name;
    });
    ...
    const rangeData = sorted.map((m: any) => {
    ...
    const allTps = sorted.map((m: any) => m.tokensPerSecond || 0);
    const tMin = Math.min(...allTps);
    const tMax = Math.max(...allTps);
    const tSpan = tMax - tMin || 1;

    function tpsColor(tps: any, alpha = 0.55) {
      ...
      plugins: [
        makeDatalabelsPlugin({
          getLabel: (_raw: any, i: any) => {
            const m = sorted[i];
            if (!m) return "";
            const range = (tpsRanges as any)[(m as any).displayName];
            if (range && range.count > 1) {
              return `${range.min.toFixed(0)}–${range.max.toFixed(0)} t/s`;
            }
            return `${((m as any).tokensPerSecond || 0).toFixed(0)} t/s`;
          },
          anchor: "end",
          align: "right",
          offset: 6,
          ...
          tooltip: {
            ...
            callbacks: {
              title: (items: any) => {
                ...
                return (sorted[item.dataIndex] as any)?.displayName || "";
              },
              afterTitle: (items: any) => {
                const item = items[0];
                if (!item) return "";
                ...
                if (!m) return "";
                return `${(m as any).quantization} · ${(m as any).architecture} · ${((m as any).contextLength / 1024).toFixed(0)}K ctx`;
              },
              label: (item: any) => {
                if (item.datasetIndex === 1) {
                  const e = item.raw?.entry;
                ...
                const m = sorted[item.dataIndex];
                const range = (tpsRanges as Record<string, any>)[m?.displayName || ""];
                if (range && range.count > 1) {
  ...
  const renderQuantDist = useCallback(() => {
    ...
    for (const m of models) {
      ...
      if ((m.modelVramGiB || 0) < (quantGroups as any)[q].minVram)
        (quantGroups as any)[q].minVram = m.modelVramGiB;
      if ((m.modelVramGiB || 0) > (quantGroups as any)[q].maxVram)
    ...
    const avgLinePlugin = {
      ...
      afterDatasetsDraw(chart: any) {
        ...
        const drawAvgTick = (datasetIdx: any, scaleId: any, getAvg: any) => {
          ...
          for (let i = 0; i < meta.data.length; i++) {
            ...
            const avg = getAvg((quantGroups as any)[q]);
  ...
  const renderCtxLeaderboard = useCallback(() => {
    ...
    const ctxRangeData = sorted.map((m: any) => {
      const range = (ctxRanges as any)[m.displayName];
      if (range && range.count > 1) {
        return [range.min, range.max];
      }
    ...
    // Build floating bar data: [min, max] TPS tuples per model
    const tpsRangeData = sorted.map((m: any) => {
      const range = (tpsRanges as any)[m.displayName];
      if (range && range.count > 1) {
        return [range.min, range.max];
      }
    ...
    function ctxColor(k: any, alpha = 0.55) {
        ...
        makeDatalabelsPlugin({
          getLabel: (_raw: any, i: any) => {
            ...
            if (tRange && tRange.count > 1) {
              ...
              tpsLabel = `${((m as any).tokensPerSecond || 0).toFixed(0)} t/s`;
            }
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (item: any) => {
                ...
                return ` Speed: ${(m as any).tokensPerSecond?.toFixed(1) || "0"} tok/s`;
              },
              afterBody: (items: any) => {
                const item = items[0];
                if (!item) return "";
  ...
  const renderContext = useCallback(() => {
    ...
    for (const d of allFilteredData) {
      const modelName = (d as any).displayName;
      const hostname = (d as any).system?.hostname || "unknown";
    ...
    const sortedGroups = Object.entries(groups)
      .map(([key, { modelName, hostname, ctxMap }]: any) => ({
        key,
        modelName,
        hostname,
    ...
    const datasets = sortedGroups.map(({ modelName, items }: any) => {
      const color = (modelColorMap as any)[modelName];
      const lineIdx = (modelLineCount as any)[modelName] || 0;
      (modelLineCount as any)[modelName] = lineIdx + 1;
        ...
        scales: {
          x: {
            ...
            afterBuildTicks: (axis: any) => {
              ...
              axis.ticks = pow2
                .filter((v: any) => v >= axis.min && v <= axis.max)
                .map((v: any) => ({ value: v }));
            },
            ticks: {
              ...TICK_STYLE,
  ...
  const prevViewRef = useRef<any>(activeView);
  useEffect(() => {
    const prev = prevViewRef.current;
    if (prev !== activeView) {
      destroyChart(prev);
      // Clear cached original colors for the destroyed chart
      delete (searchOrigColors.current as any)[prev];
  ...
  useEffect(() => {
    ...
    delete (searchOrigColors.current as any)[activeView];
      ...
      <div className={styles.content}>
        ...
        <div className={styles.chartCard}>
          ...
          <FilterBarComponent>
            <div className={styles.chartSearchGroup}>
              ...
              <input
                ...
                value={chartSearch}
                onChange={(e: any) => setChartSearch(e.target.value)}
              />
              {chartSearch && (
                <button
            ...
            <SelectComponent
              ...
              options={[
                ...
                ...settingsLabels.map((s: any) => ({
                  value: s,
                  label: s,
                  icon: <span>{(SETTINGS_EMOJI as any)[s] || "⚙️"}</span>,
              ...
              <div className={styles.vramClipGroup}>
                ...
                <div className={styles.vramClipInputs}>
                  <input
                    ...
                    placeholder="Min"
                    value={ctxMin}
                    onChange={(e: any) => setCtxMin(e.target.value)}
                    min="0"
                    step="1"
                  />
                  ...
                  <input
                    ...
                    placeholder="Max"
                    value={ctxMax}
                    onChange={(e: any) => setCtxMax(e.target.value)}
                    min="0"
                    step="1"
                  />
            ...
            {activeView === "scatter" && (
              <FilterSelectComponent
                ...
                options={SCATTER_MODES.map((m: any) => ({
                  value: m.key,
                  label: `Axes: ${m.label}`,
                }))}
              />
              ...
              <div className={styles.vramClipGroup}>
                ...
                <div className={styles.vramClipInputs}>
                  <input
                    ...
                    onChange={(e: any) => setScatterClipXMin(e.target.value)}
                    min="0"
                    step="0.5"
                  />
                  <span className={styles.vramClipSeparator}>–</span>
                  <input
                    ...
                    onChange={(e: any) => setScatterClipXMax(e.target.value)}
                    min="0"
                    step="0.5"
                  />
              ...
              <div className={styles.vramClipGroup}>
                ...
                <div className={styles.vramClipInputs}>
                  <input
                    ...
                    placeholder="Min"
                    value={vramClipMin}
                    onChange={(e: any) => setVramClipMin(e.target.value)}
                    min="0"
                    step="0.5"
                  />
                  <span className={styles.vramClipSeparator}>–</span>
                  <input
                    ...
                    placeholder="Max"
                    value={vramClipMax}
                    onChange={(e: any) => setVramClipMax(e.target.value)}
                    min="0"
                    step="0.5"
                  />
              ...
              <div className={styles.vramClipGroup}>
                ...
                <div className={styles.vramClipInputs}>
                  <input
                    ...
                    placeholder="Min"
                    value={tpsClipMin}
                    onChange={(e: any) => setTpsClipMin(e.target.value)}
                    min="0"
                    step="5"
                  />
                  <span className={styles.vramClipSeparator}>–</span>
                  <input
                    ...
                    placeholder="Max"
                    value={tpsClipMax}
                    onChange={(e: any) => setTpsClipMax(e.target.value)}
                    min="0"
                    step="5"
                  />
          ...
          <p className={styles.chartDescription}>
            {(chartDescriptions as any)[activeView]}
          </p>
