...
describe("ApprovalCard offers Always allow only with a context", () => {
  it("renders the control when `alwaysAllow` is given, and it approves through onApprove", async () => {
    vi.mocked(PermissionRulesService.create).mockResolvedValue({} as never);
    const onApprove = vi.fn();
    const { rerender } = render(<ApprovalCardComponent {...CALL} onApprove={onApprove} />);
    expect(screen.queryByText("Always allow…")).toBeNull();

    rerender(<ApprovalCardComponent {...CALL} onApprove={onApprove} alwaysAllow={{ conversationId: "conv-7", workspaceRoot: "/ws" }} />);
    fireEvent.click(screen.getByText("Always allow…"));
    await screen.findByTestId("always-allow-panel");
    fireEvent.click(screen.getByText("Save rule & approve"));
